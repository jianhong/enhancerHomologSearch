...
class AlignmentOutput
{
    ...
    private:
        ...
        void findRangeValues(Alignment* alignPtr, rangeNum *rnum, int firstRes, int lastRes,
                             int firstSeq);
        bool openExplicitFile(unique_ptr<ofstream>& outFile, string fileName);
        string openOutputFile(unique_ptr<ofstream>& outFile, string prompt, string path,
                              string fileExtension);
        int SeqGCGCheckSum(vector<char>* sequence, int length);
        void showAlign();
        /* Attributes */

        unique_ptr<ofstream> clustalOutFile;
        unique_ptr<ofstream> gcgOutFile;
        unique_ptr<ofstream> nbrfOutFile;
        unique_ptr<ofstream> phylipOutFile;
        unique_ptr<ofstream> gdeOutFile;
        unique_ptr<ofstream> nexusOutFile;
        unique_ptr<ofstream> fastaOutFile;
